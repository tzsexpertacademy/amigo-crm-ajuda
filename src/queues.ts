...
// const connection = "redis://default:MinhaSenhaForte123@127.0.0.1:7000"; -- usar esse e ajustar compose
...
let isAlreadyRunningVerify = false;

async function handleVerifySchedules(job) {
  if (isAlreadyRunningVerify) return;
  isAlreadyRunningVerify = true;
  try {
    const { count, rows: schedules } = await Schedule.findAndCountAll({
      where: {
        status: {
          [Op.in]: ["PENDENTE", "AGENDADA", "AGENDADA-AUX-1", "AGENDADA-AUX-2"]
        },
        [Op.or]: [
          {
            sendAt: {
              ...
              [Op.lte]: moment().tz("America/Sao_Paulo")
                .add(30, "seconds").format("YYYY-MM-DD HH:mm:ss")
            }
          },
          {
            sendAt_first_aux: {
              [Op.gte]: moment().tz("America/Sao_Paulo")
                .format("YYYY-MM-DD HH:mm:ss"),
              [Op.lte]: moment().tz("America/Sao_Paulo")
                .add(30, "minutes").format("YYYY-MM-DD HH:mm:ss")
            }
          },
          {
            sendAt_second_aux: {
    ...
    console.log("Horário atual (fuso BR):", now.format("YYYY-MM-DD HH:mm:ss"));
    ...
    if (count > 0) {
      schedules.map(async (schedule) => {
        ...
        const currentSendStatus = Array.isArray(schedule.sendStatus)
          ? schedule.sendStatus
          : typeof schedule.sendStatus === "string" && schedule.sendStatus.startsWith("[")
            ? JSON.parse(schedule.sendStatus)
            : [];

        const hasStatus = (s) => currentSendStatus.includes(s);

        // PRINCIPAL
        if (
          schedule.sendAt &&
          moment(schedule.sendAt).tz("America/Sao_Paulo").diff(now, "seconds") <= 30 &&
          !hasStatus("AGENDADA")
        ) {
          statusesToUpdate.push("AGENDADA");
          messagesToSend.push({
            status: "AGENDADA",
            body: schedule?.body ?? "Mensagem principal agendada"
          });
          console.log(`[AGENDAMENTO] Principal | ID: ${schedule.id} | Contato: ${schedule.contact.name} | Horário: ${moment(schedule.sendAt).tz("America/Sao_Paulo").format("YYYY-MM-DD HH:mm:ss")}`);
        }

        // AUXILIAR 1
        if (
          schedule.sendAt_first_aux &&
          moment(schedule.sendAt_first_aux).tz("America/Sao_Paulo").diff(now, "seconds") <= 30 &&
          !hasStatus("AGENDADA-AUX-1")
        ) {
          statusesToUpdate.push("AGENDADA-AUX-1");
          messagesToSend.push({
            status: "AGENDADA-AUX-1",
            body: schedule?.body_first_aux ?? "Mensagem auxiliar 1 agendada"
          });
          console.log(`[AGENDAMENTO] Auxiliar 1 | ID: ${schedule.id} | Contato: ${schedule.contact.name} | Horário: ${moment(schedule.sendAt_first_aux).tz("America/Sao_Paulo").format("YYYY-MM-DD HH:mm:ss")}`);
        }

        // AUXILIAR 2
        if (
          schedule.sendAt_second_aux &&
          moment(schedule.sendAt_second_aux).tz("America/Sao_Paulo").diff(now, "seconds") <= 30 &&
          !hasStatus("AGENDADA-AUX-2")
        ) {
          statusesToUpdate.push("AGENDADA-AUX-2");
          messagesToSend.push({
            status: "AGENDADA-AUX-2",
            body: schedule?.body_second_aux ?? "Mensagem auxiliar 2 agendada"
          });
          console.log(`[AGENDAMENTO] Auxiliar 2 | ID: ${schedule.id} | Contato: ${schedule.contact.name} | Horário: ${moment(schedule.sendAt_second_aux).tz("America/Sao_Paulo").format("YYYY-MM-DD HH:mm:ss")}`);
        }

        // UPDATE sendStatus COM OS NOVOS STATUS
        if (statusesToUpdate.length > 0) {
          const updatedSendStatus = JSON.stringify([...currentSendStatus, ...statusesToUpdate]);

          await schedule.update({ sendStatus: updatedSendStatus });
          console.log(`[STATUS] sendStatus atualizado para: ${updatedSendStatus} | ID: ${schedule.id}`);
        }
        ...
        for (const msg of messagesToSend) {
          sendScheduledMessages.add(
            "SendMessage",
            { schedule: { scheduleId: schedule.id, body: msg.body } },
            { delay: 4000 }
          );
          logger.info(`[MENSAGEM] Disparo (${msg.status}) agendado para ${schedule.contact.name} (ID: ${schedule.id})`);
        }
...
async function handleSendScheduledMessage(job) {
  ...
  const { scheduleId, body } = data.schedule
  let scheduleRecord = null;
  console.log('lidando com Agendamentos encontrados: ', scheduleId);
  try {
    ...
    if (!scheduleRecord) {
      logger.info(`Agendamento não encontrado: ${scheduleId}`);
      return;
    }
  } catch (e) {
    Sentry.captureException(e);
  ...
  try {
    ...
    await SendMessage(whatsapp, {
      ...
      body: formatBody(body, scheduleRecord.contact),
      mediaPath: filePath
    });
...
async function handleVerifyCampaigns(job) {
  ...
  const campaigns: { id: number; scheduledAt: string }[] =
    await sequelize.query(
      `select id, "scheduledAt" from "Campaigns" c
    where "scheduledAt" between now() - '1 hour'::interval and now() + '1 hour'::interval and status = 'PROGRAMADA'`,
      { type: QueryTypes.SELECT }
    );
  ...
  for (let campaign of campaigns) {
    try {
      ...
      const delay = scheduledAt.diff(now, "milliseconds") > 0 ? scheduledAt.diff(now, "milliseconds") : 5000;
      logger.info(
      ...
      console.log("ERROR HANDLE VERIFY CAMPAIN", error)
...
function calculateDelay(index, baseDelay, longerIntervalAfter, greaterInterval, messageInterval) {
  ...
  if (index > longerIntervalAfter) {
    result = diffSeconds * 1000 + greaterInterval
  } else {
    result = diffSeconds * 1000 + messageInterval
  }

  return result > 0 ? result : 5000
}
...
async function handleProcessCampaign(job) {
  try {
    ...
    if (campaign) {
      ...
      // console.log("[DEBUG: handleProcessCampaign] - PROMPT FOUND: ", prompt)
      let safePrompt;
      if (prompt) {
      ...
      // console.log("[DEBUG: safePrompt] - safePrompt FOUND: ", safePrompt)


      const { contacts } = campaign.contactList;
      // console.log("[DEBUG: handleProcessCampaign isArray(contacts)]", isArray(contacts))
      ...
      if (isArray(contacts)) {
        ...
        const queuePromises = [];
        // console.log("[DEBUG: handleProcessCampaign]", contacts)
        ...
        for (let i = 0; i < contactData.length; i++) {
          ...
          // console.log("[DEBUG: handleProcessCampaign]", contactId, campaignId, variables, delay, companyId, safePrompt)
          const queuePromise = campaignQueue.add(
            "PrepareContact",
            { contactId, campaignId, variables, delay, companyId, safePrompt },
        ...
        console.log("[DEBUG] Atualizando status da campanha para 'EM_ANDAMENTO'");
        await Campaign.update({ status: "EM_ANDAMENTO" }, { where: { id: campaign.id } }
        );
        console.log("[DEBUG] Status atualizado com sucesso!");
      } else {
        console.log("[DEBUG: handleProcessCampaign else isArray(contacts)]", isArray(contacts))

      }
    }
  } catch (err: any) {
    console.log("[DEBUG ERROR - handleProcessCampaign] ->", err)
    Sentry.captureException(err);
    throw err;
  }
}
...
async function handlePrepareContact(job) {
  ...
  try {
    const { contactId, campaignId, delay, variables, companyId, safePrompt }: PrepareContactData =
      job.data;
    // console.log("[DEBUG: handlePrepareContact]", contactId, campaignId, delay, variables, companyId, safePrompt)
    ...
    if (
      ...
      console.log("[DEBUG: initial DispatchCampaign]", contactId)
      ...
      const nextJob = await campaignQueue.add(
        "DispatchCampaign",
        {
          ...
          safePrompt,
          contactId,
          companyId: campaignShipping.contactId,
        },
        {
          delay
        }
    ...
    console.error(`campaignQueue -> PrepareContact -> error: ${err}`);
    throw error;
...
async function handleDispatchCampaign(job) {
  try {
    const { data } = job;
    const { campaignShippingId, campaignId, safePrompt, contactId, companyId }: DispatchCampaignData = data;
    const campaign = await getCampaign(campaignId);
    const wbot = await GetWhatsappWbot(campaign.whatsapp);

    // console.log("[DEBUG: IN QUEUE DispatchCampaign]", data)
    ...
    const client = new OpenAI({
      apiKey: safePrompt.apiKey
    });


    // 🔍 Buscar o último ticket existente
    ...
    // 🆕 Criar o ticket se não existir
    if (!ticket) {
      const [newTicket] = await Ticket.findOrCreate({
        where: {
          contactId,
          companyId: campaign.companyId ?? campaign.company.id,
          whatsappId: campaign.whatsappId ?? campaign.whatsapp.id
        },
        defaults: {
          contactId,
          companyId: campaign.companyId ?? campaign.company.id,
    ...
    if (campaign.mediaPath) {
      ...
      if (ticket.threadId) {
        console.log(`[DEBUG] Ticket ${ticket.id} já tem threadId: ${ticket.threadId}`);

        // ✉️ Adicionar mensagem na thread existente
        try {
          await client.beta.threads.messages.create(ticket.threadId, {
            role: "assistant",
            content: body
          });
        } catch (error) {
          console.log("ERROR TO ADD MESSAGE TO A THREAD")
        ...
        try {
          console.log("[DEBUG: OPENAI CREATE THREAD TICKET FOUND", id)

          const result = await client.beta.threads.create({
            messages: [payload]
          });
          console.log("[DEBUG: OPENAI CREATE THREAD result", result)

          console.log("[DEBUG: ATUALIZANDO TICKET", result?.id, id)

          // await Ticket.update(
          //   { threadId: result?.id },
          //   { where: { id: ticket.id } }
          // );
          ticket.update({ threadId: result?.id });
          ticket.save()
          console.log("[DEBUG: TICKET ATUALIZADO", result?.id, id)
        } catch (error) {
          console.log("ERROR TO CREATE THREAD")
          console.log("error:", error)
        }
    ...
    console.log("ERROR DISPATCH CAMPAIN", error);
    throw error;
  }
}
