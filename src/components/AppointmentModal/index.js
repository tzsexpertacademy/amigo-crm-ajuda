...
const AppointmentModal = ({ open, onClose, appointmentId, reload }) => {
  ...
  const handleSaveAppointment = async (values) => {
    try {
      if (appointmentId) {
        await api.put(`/appointments/${appointmentId}`, values);
      } else {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth scroll="paper">
      ...
      {appointmentId && appointment.ticket && (
        <Button
          variant="contained"
          color="primary"
          onClick={() => history.push(`/tickets/${appointment.ticket.uuid}`)}
          style={{ margin: "16px" }}
        >
          Ir para a conversa
        </Button>
