                ...
                <Form className={classes.fullWidth}>
                    <Grid spacing={1} justifyContent="flex-start" container>
                        ...
                        <Grid xs={12} sm={8} md={2} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="useSchedules-selection">Agendamento de Mensagens</InputLabel>
                                <Field
                                    as={Select}
                        ...
                        <Grid xs={12} sm={8} md={2} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="useAppointments-selection">Agenda</InputLabel>
                                <Field
                                    as={Select}
                                    id="useAppointments-selection"
                                    label={"Agenda"}
                                    labelId="useAppointments-selection-label"
...
export function PlansManagerGrid(props) {
    ...
    const renderCampaigns = (row) => {
        return !!row.useCampaigns  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderSchedules = (row) => {
        return !!row.useSchedules  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderAppointment = (row) => {
        return !!row.useAppointments  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderInternalChat = (row) => {
        return !!row.useInternalChat  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderExternalApi = (row) => {
        return !!row.useExternalApi  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderKanban = (row) => {
        return !!row.useKanban  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderOpenAi = (row) => {
        return !!row.useOpenAi  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };

    const renderIntegrations = (row) => {
        return !!row.useIntegrations  ? `${i18n.t("plans.form.yes")}` : `${i18n.t("plans.form.no")}`;
    };
            ...
            >
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="center">Agendamento de Mensagem</TableCell>
                ...
                <TableBody>
                    {records.map((row) => (
                        <TableRow key={row.id}>
                            ...
                            <TableCell align="center">{renderAppointment(row)}</TableCell>
...
export default function PlansManager() {
    ...
    const handleSelect = (data) => {
        ...
        let useCampaigns = !!data.useCampaigns 
        let useSchedules = !!data.useSchedules 
        let useAppointments = !!data.useAppointments 
        let useInternalChat = !!data.useInternalChat 
        let useExternalApi = !!data.useExternalApi 
        let useKanban = !!data.useKanban 
        let useOpenAi = !!data.useOpenAi 
        let useIntegrations = !!data.useIntegrations 
        // Da merda usar aquilo lá pois o valor pode ser null, 
        // sendo null ele vai dar como se tivesse true, pois "null" é diferente de false e com isso ele atribui "true", 
        // pois é um ternario ou seja qualquer coisa diferente de false vai ser dada como true
        setRecord({
            id: data.id,
            name: data.name || '',
            users: data.users || 0,
