...
import * as exportedFunctions from "../services/WbotServices/OpenaiFunctions";
...
export const testFunction = async(req: Request,
  res: Response)=>{
  const {body} = req;
  const {realTicket, toolCalls} = body;
    ...
    toolCalls.map(async (toolCall) => {
        ...
        if (functionName && functionName in exportedFunctions) {
            try {
                ...
                console.log(`✅ [RequiresAction] Resultado da função ${functionName}:`, result);
                ...
                console.error(`❌ [RequiresAction] Erro na execução da função ${functionName}:`, error.message);
                throw error;
            }
        } else {
            console.error(`❌ [RequiresAction] Função ${functionName} não encontrada.`);
            throw new Error(`Function ${functionName} not found.`);
        }
    })
);

return res.json(toolOutputs);
