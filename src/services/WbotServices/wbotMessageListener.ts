...
import { sendPromptMessageV2 } from "./SendPromptV2/SendPromptMessage";
...
const handleOpenAi = async (msg, wbot, ticket, contact, mediaSent) => {
  ...
  console.log("[HANDLE OPEN AI] - [TICKET STATUS] - ", ticket?.status);
  ...
  if (openAiIndex === -1) {
    ...
    sessionsOpenAi.push({ id: wbot.id, ...openai } as any);
  } else {
  ...
  if (msg.message?.imageMessage) {
    ...
    const imageUrl = `${process.env.BACKEND_URL}/public/${media.filename}`;

    console.log("[MEDIA] - ", media);

    const response = await sendPromptMessageV2(
      [
    ...
    let response = await sendPromptMessageV2(
      bodyMessage,
      ticket,
      msg,
      contact,
...
export const verifyMessage = async (
  ...
  if (!msg.key.fromMe && (ticket.status === "closed" || ticket.status === "closed-block-queue")) {
    await ticket.update({ status: "pending" });
    await ticket.reload({
      include: [
        { model: Queue, as: "queue" },
  ...
  if (queues.length === 1) {
    ...
    if (!msg.key.fromMe && !ticket.isGroup && !isNil(queues[0]?.promptId)) {
      ...
      const hasMedia =
        ...
        msg.message.stickerMessage;
      console.log("[VERIFY QUEUE]", ticket.fromMe, ticket.lastMessage);
      if (hasMedia) {
        mediaSent = await verifyMediaMessage(msg, ticket, contact);
        if (!ticket.fromMe) {
          console.log("HANDLE OPENAI 1")
    ...
    if (!prompt && !isNil(ticket?.queue?.prompt)) {
      console.log("[HANDLE OPEN AI] - [UPDATE TICKET] DE PENDING -> OPEN", ticket?.id);
      status = 'open'
    }else{
      console.log(!prompt)
      console.log(!isNil(ticket?.queue?.prompt))
    }

    await UpdateTicketService({
      ticketData: { queueId: firstQueue.id, chatbot, status },
      ticketId: ticket.id,
      companyId: ticket.companyId
    });
  ...
  try {
    wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
      ...
      messages.forEach(async (message: proto.IWebMessageInfo) => {
        ...
        if (!messageExists) {
          // console.log("CHEGOU MENSAGEM!", message, wbot)
