import { PromptMessageQueue, DelayPromptQueue } from "../PromptMessageQueue";

export const delayPromptHandler = async (job) => {
  const { body, ticketId, companyId, promptKey, msgData, contactId, promptId } = job.data;

  console.log(`⏳ [DELAY] - Delay de 15 segundos concluído. Verificando prioridade para o ticket: ${ticketId}`);
